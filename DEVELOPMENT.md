...
The unit tests run entirely offline: [test/zteClient.test.js](test/zteClient.test.js)
...
`SHA256( SHA256(password) + LD )`. See [lib/zteClient.js](lib/zteClient.js).
...
the "web UI has priority" back-off live in `poll()` in [main.js](main.js).
...
2. Add or adjust the entry in [lib/fields.js](lib/fields.js) (`cmd` is the raw field
   name, `id` the ioBroker state id) and extend the tests in
   [test/fields.test.js](test/fields.test.js).
...
[README.md](README.md); the release script copies it into `io-package.json`.
