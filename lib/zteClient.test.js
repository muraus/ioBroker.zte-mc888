 ...
 * @param {function} [opts.onPost]      (bodyParams) => responseObject for POST
 * @returns {Promise<{client: ZteClient, close: function, posts: object[]}>}
 */
...
describe('lib/zteClient', () => {
    describe('crypto helpers', () => {
        ...
        it('_sha256Upper returns an uppercase hex digest', () => {
            ...
            expect(out).to.equal(
                crypto.createHash('sha256').update('test', 'utf8').digest('hex').toUpperCase(),
            );
    ...
    describe('login', () => {
        ...
        it('sends the expected password hash and AD token on result=0', async () => {
            ...
            const expectedAd = c._md5Upper(
                c._md5Lower(values.cr_version + values.wa_inner_version) + values.RD,
            );
