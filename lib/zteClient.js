 ...
 *
 * No external HTTP dependency is used; the built-in `http`/`https` modules are
 * enough for the local API of the router.
 */
class ZteClient {
    /**
     * @param {string} ip        router ip, e.g. "192.168.0.1"; may carry a port ("192.168.0.1:8080")
     * @param {object} [options] client options
     * @param {number} [options.timeout] request timeout in ms (default 10000)
     * @param {'http'|'https'} [options.protocol] transport protocol (default "http")
     */
    constructor(ip, options = {}) {
        ...
        // Own agent instead of the global one: since Node 19 `globalAgent` keeps
        // connections alive, but the router closes idle sockets almost
        // immediately, so a pooled socket is usually dead by the next poll and
        // fails with ECONNRESET ("socket hang up").
        //
        // The router presents a self-signed certificate for its local web UI, so
        // certificate verification can never succeed and is turned off. The
        // connection stays inside the local network and the alternative (plain
        // HTTP) offers no protection at all.
        this.agent =
            this.protocol === 'https'
    ...
    /**
     * Base URL of the router, including the protocol and any port carried by the
     * configured address.
     *
     * @returns {string} e.g. "https://192.168.0.1"
     */
    _baseUrl() {
